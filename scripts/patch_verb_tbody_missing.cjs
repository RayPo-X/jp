...
const targetStr = `                           {verbTableColumnOrder.map(colId => {
    if (colId === 'type') {
...
const newStr = `                           {verbTableColumnOrder.map(colId => {
...
if (idx !== -1) {
    c = c.substring(0, idx) + newStr + c.substring(idx + targetStr.length);
    fs.writeFileSync('src/App.jsx', c);
    console.log('done patching verb tbody');
} else {
    console.log('failed to find verb tbody mapping block');
}
