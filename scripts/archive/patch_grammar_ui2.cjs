...
const searchStart = '<span className="ml-1 text-slate-600 font-bold flex items-center flex-wrap gap-1 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100 shadow-sm">';
const searchEnd = '})()}\n                                </span>';

let idxStart = c.indexOf(searchStart);
let idxEnd = c.indexOf(searchEnd, idxStart);
...
if (idxStart !== -1 && idxEnd !== -1) {
  ...
  c = c.substring(0, idxStart) + replaceStr + c.substring(idxEnd + searchEnd.length);
