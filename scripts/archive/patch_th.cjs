...
// 1. Fix Vocab Table
content = content.replace('<table className="w-full text-left text-sm">', '<table className="w-full text-left text-sm table-fixed">');
...
const vOldTrim = vocabThOld.replace(/\s+/g, ' ').trim();
const vNewTrim = vocabThNew.replace(/\s+/g, ' ');

let foundVocab = false;
let foundVerb = false;

// We will do a generic replacement matching the old string structure for safety
content = content.replace(/<th key=\{colId\}[\s\S]*?<\/th>/, (match) => {
  if (match.includes('dragVocabColIdx')) {
      foundVocab = true;
      return vocabThNew;
  }
  return match;
