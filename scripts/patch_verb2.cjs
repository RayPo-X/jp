...
if (!content.includes('handleVerbSort')) {
    content = content.replace(s1, s1 + '\n' + verbSortingLogic);
}

// 2. Replace the table rendering safely by replacing the explicit block
const targetTableStart = '<thead className="bg-slate-50 text-slate-600"><tr><th className="p-4 rounded-tl-xl">類型/群組</th><th className="p-4">ます形</th><th className="p-4">辭書/て形</th><th className="p-4">中文意思</th><th className="p-4">難易度</th><th className="p-4 rounded-tr-xl">操作</th></tr></thead>';
const replacementTableStart = `<thead className="bg-slate-50 text-slate-600"><tr>
...
const targetRow = `<td className="p-4"><span className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded font-bold whitespace-nowrap">{v.type} ({v.group})</span></td>
...
const replacementRow = `<td className="p-4"><span className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded font-bold whitespace-nowrap">{v.type === 'verb' ? '動詞' : v.type === 'adj_i' ? 'i形' : 'na形'} ({v.group})</span></td>
