...
// ---------------------------------------------------------
// 1. Home Page UI Injection (Global Search + Dashboard Lite)
// ---------------------------------------------------------
...
const heroHeaderRegex = /\{\/\* Hero Header \*\/\}\s*<div className="text-center mb-10 relative">[\s\S]*?<span className="hidden sm:inline">使用說明<\/span>\s*<\/button>\s*<\/div>/;
...
if(heroHeaderRegex.test(code)){
    code = code.replace(heroHeaderRegex, newHeroHeader);
    logs.push("Hero Header Replaced");
} else { logs.push("Hero Header NOT FOUND!"); }
...
// 5.1 Vocab Table
code = code.replace(
  /<div className="flex-1">\s*<label className="block text-xs font-bold text-amber-600 mb-1 ml-1">例句 \(選填\)<\/label>\s*<input type="text" value=\{vocabEditForm\.example\}[^>]+>\s*<\/div>/,
  `$&
...
// 5.2 Verb Table
code = code.replace(
  /<div className="flex-1 min-w-\[120px\]">\s*<label className="block text-xs font-bold text-indigo-600 mb-1 ml-1">中文解釋<\/label>\s*<input type="text" value=\{verbEditForm\.meaning\}[^>]+>\s*<\/div>/,
  `$&
                                 <div className="w-full mt-2 col-span-full">
...
code = code.replace(
  ...
  (match, offset, str) => {
     // Only replace the first one (Add Grammar)
     if (str.substring(0, offset).includes("newGrammar.isImportant")) {
...
// ---------------------------------------------------------
// Original is probably: {customGrammars.filter(g => !grammarFilterTag || g.tag === grammarFilterTag).sort((a, b) => {
code = code.replace(
