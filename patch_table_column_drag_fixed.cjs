...
const stateTarget = `  useEffect(() => { localStorage.setItem('verbApp_verbForms', JSON.stringify(verbForms)); }, [verbForms]);`;
...
const oldTheadRegex = /<th className="p-4 rounded-tl-xl cursor-pointer hover:bg-slate-100 transition-colors select-none" onClick=\{\(\) => handleVerbSort\('type'\)\}>[\s\S]*?<th className="p-4 rounded-tr-xl">操作<\/th>/;
const newThead = `{verbTableColumnOrder.map((colId, idx) => {
...
const parts = c.split('sortedVerbDB.map(v => (');
if (parts.length > 1) {
  parts[1] = parts[1].replace(oldCellsRegex, newCells);
  c = parts.join('sortedVerbDB.map(v => (');
} else {
  c = c.replace(oldCellsRegex, newCells);
}

// 4. Update colSpan in edit mode
const editModeParts = c.split('setEditingVerbId(null)} className="p-2 text-slate-400 hover:bg-slate-100 rounded-lg transition-colors" title="取消"><XCircle className="w-4 h-4"/></button>');
if (editModeParts.length > 1) {
    editModeParts[1] = editModeParts[1].replace(/<td colSpan=\{5 \+ verbForms\.length\} className="p-4">/, '<td colSpan={verbTableColumnOrder.length} className="p-4">');
    c = editModeParts.join('setEditingVerbId(null)} className="p-2 text-slate-400 hover:bg-slate-100 rounded-lg transition-colors" title="取消"><XCircle className="w-4 h-4"/></button>');
}

fs.writeFileSync('src/App.jsx', c);
console.log('done patch_table_column_drag_fixed');
