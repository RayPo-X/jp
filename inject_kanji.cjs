...
const oldBatchInputRow = `<input type="text" placeholder="平假名 (例: たべる)" value={item.reading}`;
...
if (code.includes(oldBatchInputRow) && !code.includes('handleFetchKanji(idx)')) {
    // We need to replace the exact input line
    const regex = /<input type="text" placeholder="平假名 \(例: たべる\)" value=\{item\.reading\}[^>]+>/;
    code = code.replace(regex, newBatchInputRow);
}
